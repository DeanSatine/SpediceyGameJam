...
public class EvilManController : MonoBehaviour
{
    ...
    private SimpleCharacterController playerMovement;
    ...
    void Start()
    {
        ...
        if (player != null)
        {
            playerMovement = player.GetComponent<SimpleCharacterController>();
    ...
    private IEnumerator ExplosiveLaunch(CharacterController controller, Vector3 direction, float launchForce, float upwardForce, float drag, float gravity)
    {
        ...
        Vector3 velocity = direction * launchForce + Vector3.up * upwardForce;

        // Disable player control during launch
        SimpleCharacterController playerMove = controller.GetComponent<SimpleCharacterController>();
        if (playerMove != null) playerMove.enabled = false;

        float elapsed = 0f;
        while (!controller.isGrounded || elapsed < 0.25f)
        {
        ...
        // Land VFX impact when touching ground
        SafePlayParticleSystemAt(playerDamageEndVFX, controller.transform.position + Vector3.up * 0.5f, Quaternion.identity);
        SafeCameraShake(0.6f, 0.5f);
